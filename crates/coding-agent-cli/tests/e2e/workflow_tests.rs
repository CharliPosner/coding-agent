...
async fn test_workflow_new_session_conversation() {
    ...
    server.mock_multi_turn_conversation(vec![
    ...
    let response = session.expect_response(Duration::from_secs(10))
        .expect("Failed to get response");
    assert!(response.contains("Claude") || response.contains("assistant"),
            "Response doesn't contain expected content: {}", response);

    // 4. Send follow-up, verify context retained
    session.send_message("What is 2 + 2?").expect("Failed to send follow-up");
    let response2 = session.expect_response(Duration::from_secs(10))
        .expect("Failed to get follow-up response");
    assert!(response2.contains("4"),
            "Follow-up response doesn't contain expected answer: {}", response2);

    // 5. Check /cost shows token usage
    let cost_output = session.run_command("/cost")
        .expect("Failed to run /cost command");
    assert!(cost_output.contains("tokens") || cost_output.contains("Token"),
            "Cost output doesn't show token info: {}", cost_output);
...
async fn test_workflow_resume_session() {
    ...
    // First session
    {
        ...
        session.expect_startup_screen().expect("No startup screen");
        session.select_new_session().expect("Failed to select new session");
        session.send_message("Let's talk about Rust").expect("Failed to send message");
    ...
    // Second session - resume
    {
        ...
        session.session_mut().send_line("r").expect("Failed to send 'r'");
        session.session_mut().expect(">").expect("Failed to wait for prompt");

        // Send follow-up that requires context
        session.send_message("Can you explain lifetimes?").expect("Failed to send follow-up");
        let response = session.expect_response(Duration::from_secs(10))
            .expect("Failed to get response");

        assert!(response.contains("Lifetime") || response.contains("lifetime") ||
                response.contains("memory"),
                "Response doesn't show context awareness: {}", response);
...
async fn test_workflow_tool_execution() {
    ...
    server.mock_tool_call(
        "read_file",
        serde_json::json!({"path": test_file.to_str().unwrap()})
    ).await;

    // Then mock the response after tool execution
    server.mock_simple_response("The file contains: 'Hello from test file!'").await;
    ...
    session.send_message(&format!("Read the file at {}", test_file.display()))
    ...
    assert!(response.contains("Hello") || response.contains("test file"),
            "Response doesn't mention file contents: {}", response);
...
async fn test_workflow_multi_tool_sequence() {
    ...
    server.mock_simple_response("I've read the file and updated it with new content.").await;
    ...
    let response = session.expect_response(Duration::from_secs(15))
        .expect("Failed to get response");

    // Verify the response indicates both operations happened
    assert!(response.contains("read") || response.contains("updated") ||
            response.contains("modified"),
            "Response doesn't indicate operations completed: {}", response);
...
async fn test_workflow_git_commit() {
    ...
    fs::write(&test_file, "fn new_feature() { println!(\"Hello\"); }").expect("Failed to create file");
    ...
    // Run /commit command
    let output = session.run_command("/commit").expect("Failed to run /commit");

    // Verify the command processed changes
    assert!(output.contains("commit") || output.contains("changes") ||
            output.contains("feature"),
            "/commit output doesn't indicate processing: {}", output);
...
async fn test_workflow_permission_prompt() {
    ...
    session.send_message(&format!("Write 'Test content' to {}", untrusted_path.display()))
        .expect("Failed to send message");
    ...
    session.session_mut().set_expect_timeout(Some(Duration::from_secs(10)));
    ...
    if let Ok(output) = result {
        ...
        assert!(has_permission_text,
                "Expected permission-related content, got: {}", output_str);
    }
...
async fn test_workflow_error_handling() {
    ...
    if let Ok(output) = result {
        ...
        assert!(output_str.contains("error") ||
                ...
                "Expected error message, got: {}", output_str);
    }

    // CLI should still be responsive
    let help_output = session.run_command("/help")
        .expect("CLI not responsive after error");
    assert!(help_output.contains("help") || help_output.contains("command"),
...
async fn test_workflow_context_tracking() {
    ...
    session.send_message("First message").expect("Failed to send first message");
    let _ = session.expect_response(Duration::from_secs(10));

    session.send_message("Second message").expect("Failed to send second message");
    let _ = session.expect_response(Duration::from_secs(10));

    session.send_message("Third message").expect("Failed to send third message");
    let _ = session.expect_response(Duration::from_secs(10));

    // Check context tracking
    let context_output = session.run_command("/context")
        .expect("Failed to run /context");

    // Should show token usage
    assert!(context_output.contains("token") || context_output.contains("Token"),
            "Context output doesn't show tokens: {}", context_output);

    // Check cost tracking
    let cost_output = session.run_command("/cost")
        .expect("Failed to run /cost");

    // Should show accumulated costs
    assert!(cost_output.contains("token") || cost_output.contains("Token"),
            "Cost output doesn't show tokens: {}", cost_output);
...
async fn test_workflow_command_history() {
    let (server, api_url) = setup_test_env().await;
    server.mock_simple_response("Response to your message.").await;
    ...
    session.send_message("Test message").expect("Failed to send message");
    let _ = session.expect_response(Duration::from_secs(10));

    // Check history command
    let history_output = session.run_command("/history")
        .expect("Failed to run /history");

    // Should show past sessions or indicate history is available
    assert!(history_output.contains("history") ||
            history_output.contains("session") ||
            history_output.contains("Session"),
            "History output unexpected: {}", history_output);
...
async fn test_workflow_clear_command() {
    ...
    // Have a conversation
    session.send_message("Remember this: banana").expect("Failed to send message");
    let _ = session.expect_response(Duration::from_secs(10));

    // Clear the context
    let _clear_output = session.run_command("/clear")
    ...
    session.send_message("What did I tell you to remember?").expect("Failed to send message");
    let _response = session.expect_response(Duration::from_secs(10))
