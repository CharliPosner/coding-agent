...
### Checklist
- [ ] Modify `format_result_collapsible()` case for read_file (lines 80-85)
- [ ] Add line count check against `collapse_threshold`
- [ ] Return `FormattedResult` with `collapsed_content: Some()` when exceeding threshold
- [ ] Show summary: `"▸ file.rs (N lines, use /results to expand)"`
- [ ] Verify `/results` command expands the collapsed content
